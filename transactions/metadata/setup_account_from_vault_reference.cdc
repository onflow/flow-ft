...
transaction(address: Address, publicPath: PublicPath) {
    ...
    prepare(signer: auth(SaveValue, Capabilities) &Account) {
        ...
        let resolverRef = getAccount(address)
            ...
            ?? panic("Could not borrow a reference to the ViewResolver in account \(address) at path \(publicPath). Make sure you are querying an address that has a Vault set up properly.")
