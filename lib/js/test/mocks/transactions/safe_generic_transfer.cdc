...
transaction(amount: UFix64, to: Address, senderPath: StoragePath, receiverPath: PublicPath) {
    ...
    prepare(signer: AuthAccount) {
        ...
        let vaultRef = signer.borrow<&{Token.Provider}>(from: senderPath)
			?? panic("Could not borrow reference to the owner's Vault!")
        
        self.senderReceiverRef = signer.borrow<&{Token.Receiver}>(from: senderPath)
			?? panic("Could not borrow {Token.Receiver} reference to the owner's Vault!")
