...
transaction(receiver: Address) {
    ...
    prepare(acct: AuthAccount) {
        ...
        let recipient = getAccount(receiver)
            .getCapability<&{Token.Receiver}>(TestToken.ReceiverPublicPath)
        ...
        acct.save(<-vault, to: /storage/testTokenForwarder)

        // Unlink the existing receiver capability
        if acct.getCapability(TestToken.ReceiverPublicPath).check<&{Token.Receiver}>() {
            acct.unlink(TestToken.ReceiverPublicPath)
        }

        // Link the new forwarding receiver capability
        acct.link<&{Token.Receiver}>(
            TestToken.ReceiverPublicPath,
            target: /storage/testTokenForwarder
        )
    }
}