...
import { emulator, init, getAccountAddress, deployContractByName, sendTransaction, shallPass, 
  executeScript } from "@onflow/flow-js-testing";
  import fs from "fs";
...
describe("exampletoken", ()=>{
  ...
  beforeEach(async () => {
    ...
    await deployContract({ to: serviceAccount,    name: "utilityContracts/NonFungibleToken"});
    await deployContract({ to: serviceAccount,    name: "utilityContracts/MetadataViews"});
  ...
  // Fifth test uses a vault as a resolver to get its FTView
  test("should be able to retrieve the FTView from a vault", async () => {
    await shallPass(
      sendTransaction({
        name: "setup_account",
        args: [],
    ...
    await shallPass(
      sendTransaction({
        name: "setup_account_from_vault_reference",
        args: [exampleTokenUserA, "/public/exampleTokenReceiver"],
        signers: [exampleTokenUserB]
      })
    );
  })
